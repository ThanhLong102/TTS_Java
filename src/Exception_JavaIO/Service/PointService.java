...
import Exception_JavaIO.Entity.Point.SubjectPoint;
...
public class PointService {
    ...
    public void creatPointTable() {
        ...
        PointTable pointTable = findPointTable(student.getId());
        ...
        if (DataUtil.isNullOrEmpty(pointTable)) {
            ...
            updatePointTableToArray(new PointTable(student, subjectPoints), findIndexPointTableExisted(student.getId()));
        }
        Menu.fileUtil.writeDataToFile(Menu.pointTables, POINT_DATA_FILE);
    }

    private void updatePointTableToArray(PointTable pointTable, int indexPointTableExisted) {
        Menu.pointTables[indexPointTableExisted] = pointTable;
    }
    ...
    private void updateOrAddSubjectPointExits(SubjectPoint[] subjectPoints, int subjectNumber, Student student) {
        for (int j = 0; j < subjectNumber; j++) {
            ...
            int index = findIndexSubjectPointExisted(subject.getId(), student.getId());
    ...
    private void addPointTableToArray(PointTable pointTable) {
        for (int k = 0; k < Menu.pointTables.length; k++) {
            if (DataUtil.isNullOrEmpty(Menu.pointTables[k])) {
                Menu.pointTables[k] = pointTable;
                break;
            }
        }
    }
    ...
    private float inputSubjectPointNumber(Subject subject, Student student) {
        System.out.print("Nhập số điểm của môn " + subject.getName() + " mà sinh viên " + student.getFullName() + " đã có(hoặc update): ");
    ...
    private Student inputStudentId() {
        ...
        do {
            try {
                ...
                student = Menu.studentService.findStudentById(studentId);
    ...
    public void showPointTable() {
        for (int i = 0; i < Menu.pointTables.length; i++) {
            ...
            System.out.println(Menu.pointTables[i]);
        }
    }

    public void sortPointTableToNameStudent() {
        for (int i = 0; i < Menu.pointTables.length - 1; i++) {
            if (DataUtil.isNullOrEmpty(Menu.pointTables[i]))
                break;
            for (int j = i + 1; j < Menu.pointTables.length; j++) {
                if (DataUtil.isNullOrEmpty(Menu.pointTables[j]))
                    break;
                int compare = compareNameStudent(Menu.pointTables[i], Menu.pointTables[j]);
                if (compare > 0) {
                    swapPointTable(i, j);
                }
            }
    ...
    public void sortPointTableToNameSubject() {
        for (int i = 0; i < Menu.pointTables.length; i++) {
            if (DataUtil.isNullOrEmpty(Menu.pointTables[i]))
                break;
            for (int j = 0; j < Menu.pointTables[i].getSubjectPoints().length - 1; j++) {
                if (DataUtil.isNullOrEmpty(Menu.pointTables[i].getSubjectPoints()[j]))
                    break;
                for (int k = 0; k < Menu.pointTables[i].getSubjectPoints().length; k++) {
                    if (DataUtil.isNullOrEmpty(Menu.pointTables[i].getSubjectPoints()[k]))
                        break;
                    int compare = compareNameSubject(Menu.pointTables[i].getSubjectPoints()[j]
                            , Menu.pointTables[i].getSubjectPoints()[k]);
                    if (compare > 0) {
    ...
    private int compareNameSubject(SubjectPoint o1, SubjectPoint o2) {
        String[] ten1 = o1.getSubject().getName().split("\\s+");
        String[] ten2 = o2.getSubject().getName().split("\\s+");
        if (ten1[ten1.length - 1].equalsIgnoreCase(ten2[ten2.length - 1])) {
            return o1.getSubject().getName().compareToIgnoreCase(o2.getSubject().getName());
        } else {
    ...
    private void swapPointTable(int i, int j, int k) {
        SubjectPoint tempSubjectPoint = Menu.pointTables[i].getSubjectPoints()[j];
        Menu.pointTables[i].getSubjectPoints()[j] = Menu.pointTables[i].getSubjectPoints()[k];
        Menu.pointTables[i].getSubjectPoints()[k] = tempSubjectPoint;
    }


    private void swapPointTable(int i, int j) {
        PointTable tempPointTable = Menu.pointTables[i];
        Menu.pointTables[i] = Menu.pointTables[j];
        Menu.pointTables[j] = tempPointTable;
    }
    ...
    public void pointCalculation() {
        for (int i = 0; i < Menu.pointTables.length; i++) {
            ...
            System.out.println(Menu.pointTables[i]);
            System.out.println("Có điểm Tổng kết:" + Menu.pointTables[i].getAvgPoint());
        }
    }
    ...
    public void initializePointTableData() {
        ...
        if (!DataUtil.isNullOrEmpty(pointTableFromFile)) {
            ...
            System.arraycopy(copyPointTables, 0, Menu.pointTables, 0, copyPointTables.length);
        }
    }
}
